...
def test_password(user, password):
    print 'test_password'
    blackList = [';', 'script', 'password', '-- ']
    for x in blackList: 
    	if x in user: return False
    if not user: return False
    conn = sqlite3.connect(DBFN)
    c    = conn.cursor()
    c.executescript("SELECT password from accounts where username='%s'" % user).fetchone() #vulnerbale?
...
def post_squig(user, squig):
    ...
    blackList = [';', '-- ', '<script>', '</script>', '/logout', '/do_squig', '%22do_squig']
    for x in blackList:
    	if x in squig:
		print 'Hacking Attempt Provoked' 
		return
    conn = sqlite3.connect(DBFN)
    c    = conn.cursor()
...
<form action="do_login" method="get" id="loginform">
    ...
    <div><input name="password"  id="password" type="text" size=40 style="font-size:150%;" /></div>
